...
function matchesActiveHistoryFallback(coffee, ref) {
    if (!coffee || !ref) return false;
    if (ref.id && coffee.id && ref.id === coffee.id) return true;

    const sameAddedDate = Boolean(ref.addedDate) && Boolean(coffee.addedDate) && ref.addedDate === coffee.addedDate;
    const sameName = Boolean(ref.name) && Boolean(coffee.name) && ref.name === coffee.name;

    return sameAddedDate && sameName;
}
...
function resolveActiveHistoryCoffeeIndex() {
    ...
    if (Number.isInteger(activeHistoryCoffeeRef.index) && activeHistoryCoffeeRef.index >= 0 && activeHistoryCoffeeRef.index < coffees.length) {
        const fallbackCoffee = coffees[activeHistoryCoffeeRef.index];
        if (matchesActiveHistoryFallback(fallbackCoffee, activeHistoryCoffeeRef)) {
